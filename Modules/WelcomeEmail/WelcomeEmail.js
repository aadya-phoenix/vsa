var app = angular.module('WelcomeEmailModule', []);

app.service('WelcomeEmailService', function ($http, $location, $rootScope) {
...
app.controller('WelcomeEmailController', function ($scope, $http, $location, WelcomeEmailService,
  ...
  $scope.GetDataOnInit = function () {
    WelcomeEmailService.GetStaticContent('WelcomeEmail').then(function (success) {
  ...
  $scope.UpdateRule = function () {
    $scope.Content.DataType = 'WelcomeEmail';
    WelcomeEmailService.UpsertStaticContent($scope.Content).then(function (success) {
      swal(success.data);
    }, function (error) {
      console.log(error);
    });
